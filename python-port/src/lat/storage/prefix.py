...
def logic_app_prefix(la_name: str) -> str:
    """Production LA prefix — lowercases input to match `Common.cs:38,43`.

    NOTE: This differs from `Tools/GeneratePrefix.cs:13` and the standalone
    `Tools GeneratePrefix` CLI command, which call `Generate()` on the bare
    input. Use `generate()` directly if you need that behavior.
    """
    return generate(la_name.lower())


def workflow_prefix(flow_id: str) -> str:
    """Production workflow prefix — lowercases input to match `Common.cs:45`.

    NOTE: `Operations/GenerateTablePrefix.cs:31` and `Tools/GeneratePrefix.cs:23`
    do NOT lowercase the workflowID; use `generate()` directly for parity with
    those debug paths.
    """
